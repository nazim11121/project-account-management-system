...
<div class="col-md-12">
  <h4 class="fw-bold py-3 mb-4">
    <span class="text-muted fw-light">Account / Bank-Cash Account/ </span> List
    <a href="{{ route('account-bank-cash-create') }}" class="btn btn-primary float-right">Create</a>
  </h4>
</div>  
  ...
  <div class="card-body table-responsive">
    <table id="example" class="table table-hover table-striped table-bordered">
      ...
      <tbody class="table-border-bottom-0">
        ...
        <?php foreach ($bankCash as $key => $value): ?>
          <tr>  
        		<td>{{ ++$key }}</td>
        		<td><i class="fab fa-angular fa-lg text-danger"></i> <strong>{{ $value->name }}</strong></td>
            <td><i class="fab fa-angular fa-lg text-danger"></i> <strong>{{ $value->current_balance }}</strong></td>
            <td><i class="fab fa-angular fa-lg text-danger"></i> <strong>{{ $value->description }}</strong></td>
        		<td>
        		  <div class="dropdown">
        		    <button type="button" class="btn p-0 dropdown-toggle hide-arrow" data-bs-toggle="dropdown"><i class="bx bx-dots-vertical-rounded"></i></button>
        		    <div class="dropdown-menu">
        		      <a class="dropdown-item" href="{{ route('account-bank-cash-edit',$value->id) }}"><i class="bx bx-edit-alt me-1"></i> Edit</a>
        		      <!-- <form action="{{ route('account-bank-cash-destroy', $value->id)}}" method="post">
        		        @csrf
        		        @method('DELETE')
