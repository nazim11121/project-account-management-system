...
<div class="row">
  <div class="col-md-12">
    <div class="card mb-4">
      ...
      <hr class="my-0">
      <div class="card-body">
        <form id="formAccountSettings" method="POST" action="{{ route('ledger-ledgerName-update',$ledgerName->id) }}" enctype="multipart/form-data">
          @csrf
          @method('PATCH')
          <div class="row">
            <div class="mb-3 col-md-6">
              <label for="name" class="form-label">Name</label>
              <input class="form-control" type="text" id="name" name="name" value="{{ $ledgerName->name }}" />
            </div>
            <div class="mb-3 col-md-6">
              ...
              <select name="ledger_type" class="form-control custom-select">
                  <option value="">Select One..</option>
                    @foreach($ledger_type as $value)
                      <option value="{{ $value->id }}" @if($value->id == $ledgerName->ledger_type) selected @endif>{{ $value->type_name }}</option>
                    @endforeach
              </select>
            </div>
            <div class="mb-3 col-md-6">
              ...
              <select name="ledger_group" class="form-control custom-select">
                  <option value="">Select One..</option>
                    @foreach($ledger_group as $value)
                      <option value="{{ $value->id }}" @if($value->id == $ledgerName->ledger_group) selected @endif>{{ $value->group_name }}</option>
                    @endforeach
              </select>
            </div>
            <div class="mb-3 col-md-6">
